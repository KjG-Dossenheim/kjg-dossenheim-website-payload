...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const post = blogPosts[0] as BlogPost
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${process.env.SITE_URL}/api/og/?title=${encodeURIComponent(post.title)}?author=${encodeURIComponent(
            post.author && typeof post.author !== 'string' ? post.author.firstName : 'Unknown',
          )}?publishedDate=${encodeURIComponent(post.publishedDate || new Date().toISOString())}`,
          alt: post.title,
        },
      ],
    },
