...
export const AuthProvider: React.FC<{ api?: 'gql' | 'rest'; children: React.ReactNode }> = ({
  ...
  const create = useCallback<Create>(
    async (args) => {
      if (api === 'rest') {
        const user = await rest(`${process.env.NEXT_PUBLIC_SITE_URL}/api/sommerfreizeitUser`, args)
        setUser(user)
        return user
      }
    ...
    async (args) => {
      if (api === 'rest') {
        const user = await rest(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/sommerfreizeitUser/login`,
          args,
        )
        setUser(user)
        return user
  ...
  const logout = useCallback<Logout>(async () => {
    if (api === 'rest') {
      try {
        await rest(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/sommerfreizeitUser/logout`,
          {},
          { method: 'POST' },
        )
      } catch (error) {
        // Logout endpoint might return various responses, but we should clear the user anyway
  ...
  useEffect(() => {
    const fetchMe = async () => {
      if (api === 'rest') {
        const user = await rest(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/sommerfreizeitUser/me`,
          {},
          {
            method: 'GET',
          },
  ...
  const forgotPassword = useCallback<ForgotPassword>(
    async (args) => {
      if (api === 'rest') {
        const user = await rest(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/sommerfreizeitUser/forgot-password`,
          args,
        )
        setUser(user)
        return user
  ...
  const resetPassword = useCallback<ResetPassword>(
    async (args) => {
      if (api === 'rest') {
        const user = await rest(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/sommerfreizeitUser/reset-password`,
          args,
        )
        setUser(user)
        return user
