...
Imports Pretix customers into `sommerfreizeitUsers`.

**What it does:**

- Fetches customers from `GET /api/v1/organizers/{organizer}/customers/`
- Creates missing Sommerfreizeit accounts via Better Auth
- Optionally updates existing accounts (`updateExisting=true`)
- Supports pagination and optional filtering by one email address

**Required environment variables:**

- `NEXT_PUBLIC_PRETIX_ORGANIZER` (Pretix organizer slug)
- `NEXT_PUBLIC_PRETIX_API_TOKEN` (API token with customer read access)

**Optional environment variable:**

- `NEXT_PUBLIC_PRETIX_URL` (defaults to `https://pretix.eu`)
...
- `email` (string): import only one customer by email
- `maxPages` (number): stop import after this number of pages
- `updateExisting` (boolean): update names/phone/emailVerified for existing accounts
